export const GREEN = [133, 203, 51];

export const CELL = 22;
export const DOT = 3;
export const BASE = 0.05;
export const WAVE = 0.07;

export const BLOB = 0.55;
export const BLOB_RADIUS = 200;

export const POINTER = 0.45;
export const POINTER_RADIUS = 140;

export const BLOBS = [
    { fx: 0.31, fy: 0.23, px: 0.0, py: 1.3, ax: 0.42, ay: 0.4 },
    { fx: 0.24, fy: 0.34, px: 2.1, py: 0.4, ax: 0.4, ay: 0.42 },
    { fx: 0.38, fy: 0.27, px: 4.0, py: 3.1, ax: 0.38, ay: 0.4 },
    { fx: 0.27, fy: 0.4, px: 1.2, py: 5.0, ax: 0.41, ay: 0.38 },
];
